...
Execute plan by dispatching a fresh implementer subagent per task, a task review (spec compliance + code quality) after each, strict review gates and a broad whole-branch review at the end.
...
1. Invoke `extract-boundary` to gather minimal context for the task's files, semantic understanding and project context.
...
relevant subagents; the per-task review loop below is unchanged by it.
...
- Never dispatch multiple implementation subagents in parallel unless they
  form a file-disjoint wave (see Parallel Waves) — overlapping tasks conflict.
...
Also flip the task's checkbox in the plan file from `- [ ]` to `- [x]`, and
if a `state.md` with a plan status section exists, update it too — the
ledger is your recovery map, the plan file is what your human partner reads.

## Parallel Waves (for file-disjoint tasks)

The task loop above is sequential by default. When several tasks are
...
and the preferred mode whenever the plan's tasks actually qualify.

**Decision rule:** before execution, group tasks into waves by file overlap
and state dependency. Tasks with no shared files and no sequential
dependency belong in the same wave. Any overlap or shared-state risk moves
the conflicting task to the next wave — when in doubt, keep it sequential.
...
3. Each task in the wave keeps its own brief, report, review, and fix loop — the wave changes dispatch timing, nothing else.
4. Run integration verification (verify-all) after the wave completes.
5. Append each task's completion line to the ledger and update the plan checkboxes.
