...
BOOTSTRAP_MARKER = "superpowers-prepared:using-superpowers bootstrap for hermes"


def _skills_dir() -> str:
    """Locate the stock skills/ tree for either supported install layout.

    - git-clone install (`hermes plugins install josuerf/superpowers-prepared`): the plugin
    ...
    for cand in candidates:
        if os.path.isfile(os.path.join(cand, "using-superpowers", "SKILL.md")):
            return cand
    raise RuntimeError(
        "superpowers-prepared plugin: cannot find the skills/ tree "
        f"(looked at {candidates}). Reinstall with "
        "`hermes plugins install josuerf/superpowers-prepared`."
    )
...
def _build_bootstrap(skills_dir: str) -> str:
    ...
    return (
        ...
        f'invoke one with `skill_view("superpowers-prepared:skill-name")` '
        f'(for example `skill_view("superpowers-prepared:brainstorming")`). '
