...
main() {
    ...
    # macOS mktemp lives under /var -> /private/var). On Git Bash for Windows
    # rev-parse prints a Windows path (C:/...) while the helpers print an MSYS
    # one (/c/...), so pass it through the same `cd && pwd` the helpers use.
