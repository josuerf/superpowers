#!/usr/bin/env bash
# Validate the Devin CLI integration. `devin plugins install josuerf/superpowers-prepared`
...
if manifest.get("name") != "superpowers-prepared":
    raise AssertionError(f"plugin name: expected 'superpowers-prepared', got {manifest.get('name')!r}")
